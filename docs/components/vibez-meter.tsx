...
// ==========================================
// 1. VibezReceipt: Clean, Compact Micro-Badge
// ==========================================

export interface VibezReceiptProps {
  /** Message object from Vercel AI SDK */
  message?: any;
  /** Explicit model name */
  model?: string;
  /** Total tokens */
...
export function VibezReceipt({
  ...
  if (message?.annotations && Array.isArray(message.annotations)) {
    for (const ann of message.annotations) {
      if (ann && typeof ann === 'object') {
        if (ann.type === 'vibezcheck' || ann.tokens || ann.costUSD || ann.usage || ann.cost) {
          resolvedTokens = resolvedTokens ?? ann.tokens ?? ann.usage?.totalTokens;
          resolvedCost =
            resolvedCost ??
            ann.costUSD ??
            ann.cost?.totalUSD ??
            (typeof ann.cost === 'number' ? ann.cost : undefined);
          resolvedModel = resolvedModel ?? ann.model;
          resolvedReasoning = resolvedReasoning ?? ann.reasoningTokens ?? ann.usage?.reasoningTokens;
  ...
  const formattedCost =
    resolvedCost !== undefined
      ? resolvedCost < 0.0001
        ? '<$0.0001'
        : `$${resolvedCost.toFixed(4)}`
      : null;

  const cleanName = sanitizeModel(resolvedModel);
  ...
  return (
    ...
    >
      ...
      {formattedCost && resolvedTokens !== undefined && (
...
// ==========================================
// 2. VibezCheck: Floating Pill & Attached HUD
// ==========================================
...
export function VibezCheck({
  ...
  // Dynamic Theme Observer: Reacts to dark/light toggle in Header immediately
  useEffect(() => {
  ...
  return (
    <div style={positionStyles} className={`vibezcheck-root font-sans select-none ${className}`}>
